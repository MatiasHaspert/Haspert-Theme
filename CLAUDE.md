...
- **Paleta Ultramar (v2 · jul 2026).** Los tokens `--nc-*` del `:root` de `assets/base.css` son la **única fuente de verdad** de color: acento `#1E34A8` (`--nc-accent`; deep `#16267E`, press `#101C5E`, soft `#DFE6F8`), tinta `#14161A`, fondo frío `#F7F8F9`, semánticos `--nc-ok #1E6B4E` / `--nc-alert #B02E2E`. Reglas duras: **el acento ocupa ≤ ~5% de la vista** (CTA, links, micro-badges) — derramado, el azul vira a fintech; **nunca detrás ni pegado a un packshot** (hay frascos azules en catálogo: la separación la hace el marco de card `--nc-card-frame`, no el color); **franjas oscuras (`#14161A`) solo como componente, nunca fondo de página**; **sin dorado**; **verde nunca fluo** (reservado al beneficio transferencia — no puede ganarle peso óptico al CTA); **sombras siempre frías** (3 tintes: `--nc-shadow-neutral-rgb` / `--nc-shadow-deep-rgb` / `--nc-scrim-rgb`; nunca marrón). Si el glow del CTA "vibra" en OLED de gama media, bajá el alpha `.22 → .16` **en el token `--nc-shadow-btn`**, no en el componente. Aliases `--nc-terra` / `--nc-terra-deep` / `--nc-cream` = compatibilidad **deprecada, prohibidos en código nuevo**. `--accent`/`--accent-ink`/`--accent-soft` NO son aliases del bloque: los inyecta `theme.liquid` desde settings ("Acento de marca") y **no deben redefinirse en CSS** (base.css carga después y los pisaría). Único scheme en acento: `scheme-5` (badge SALE); `scheme-3` = franja tinta. `design-system/` está en Ultramar solo en botones/badges; el resto y los mockups viejos quedaron en paleta v1 (estructura sí, colores no).
- **Tipografía (Numen): DOS fuentes, no más.** Bebas Neue (display: títulos, precios, botones; self-host) + Montserrat desde el picker (UI y cuerpo). El acento editorial es **Montserrat itálica 500**: `--font-accent` / `--font-accent-weight`, derivadas del picker con `font_modify` en `layout/theme.liquid`; todo uso de `var(--font-accent)` declara `font-style: italic` y `font-weight: var(--font-accent-weight, 500)` explícitos. **Prohibido reintroducir una tercera fuente.** Los mockups HTML del repo (`numen-home-mockup.html`, `design-system/`) todavía referencian Fraunces: son referencia histórica, no los toques ni copies su font stack.
